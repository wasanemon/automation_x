...
LLM完全自動投稿は、OpenAI Responses APIで仮説分析とdraft生成を行い、そのdraftを既存の安全gateへ通す機能です。LLMはPostizやXを直接操作しません。投稿可否はGrowth Agent側の `DraftEvaluator`、duplicate check、frequency limit、dry-run/live gate、kill switchで決まります。

初期値ではLLM生成も完全自動もoffです。
...
LLM dry-run cycleを確認する場合:

```text
OPENAI_API_KEY=<secret>
LLM_GENERATION_ENABLED=true
LLM_ANALYSIS_ENABLED=true
LLM_FULL_AUTO_ENABLED=true
AUTO_POSTING_ENABLED=false
SCHEDULING_DRY_RUN=true
AUTOMATION_KILL_SWITCH=false
```

この状態ではOpenAI APIでdraft候補を作りますが、Postiz live schedulingは呼びません。`POST /automation/run-cycle` のレスポンスで以下を確認します。

- `llm_generated_drafts_count`
- `llm_hypotheses_count`
- `llm_skipped_count`
- `dry_run_scheduled_count`
- `live_scheduled_count=0`

テスト用Xアカウントでlive schedulingまで許可する条件:
...
さらにPostizの `POSTIZ_BASE_URL`, `POSTIZ_API_KEY`, `POSTIZ_X_INTEGRATION_ID` が必要です。LLM draftでも、外部URL、短縮URL、価格/法務表現、強い主張、低confidence、model側のreview要求、duplicate、score不足、riskがlow以外の場合は自動scheduleされません。

`OPENAI_API_KEY` の実値はREADME、workflow JSON、ログ、標準出力、テストfixtureに書かないでください。
...
| `OPENAI_API_KEY` | OpenAI Responses API key。出力やdocsに出さないでください。 | LLM生成 |
| `OPENAI_API_BASE_URL` | OpenAI API base URL。default `https://api.openai.com`。 | LLM生成 |
| `OPENAI_MODEL` | LLM生成に使うmodel。default `gpt-5.5`。 | LLM生成 |
| `LLM_GENERATION_ENABLED` | `true` の時、automation cycle内でLLM draft生成を使います。 | LLM生成 |
| `LLM_ANALYSIS_ENABLED` | `true` の時、metrics summaryから仮説を生成します。 | LLM分析 |
| `LLM_FULL_AUTO_ENABLED` | `true` の時、LLM draftを人間承認なしのauto schedule候補に入れます。 | LLM自動投稿 |
| `LLM_DRAFTS_PER_CYCLE` | LLMが1 cycleで生成するdraft数。default `3`。 | LLM生成 |
| `LLM_PROMPT_VERSION` | prompt version。default `v1`。 | LLM生成 |
| `LLM_MAX_RECENT_POSTS` | LLM入力に含めるrecent/top posts上限。default `20`。 | LLM分析 |
| `LLM_MIN_CONFIDENCE` | これ未満のLLM draftはhuman approvalへ回します。default `0.7`。 | LLM安全gate |
