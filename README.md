...
Codex / ChatGPT
  -> MCP server (python -m growth_agent.mcp_server)
      -> HTTPS Growth Agent API
```
...
高度な仮説分析や投稿内容の生成は、Growth Agent server内でOpenAI APIを呼ぶのではなく、Codex / ChatGPT側の会話セッションで行います。Growth Agentは生成AIではなく、生成されたdraft候補を受け取り、保存し、既存のdeterministic safety gateを必ず通してからschedule候補にします。

この方式では `OPENAI_API_KEY` は不要です。Growth Agentに追加されているのは、ChatGPT/Codexから渡されたdraft候補を受け取る `POST /drafts/import` と、Codex/ChatGPTからGrowth Agent APIを操作しやすくするMCP serverです。
...
- `evaluate_draft`
- `run_dry_cycle`
- `explain_last_run_context`

`run_dry_cycle` は `SCHEDULING_DRY_RUN=true` の時だけ `/automation/run-cycle` を呼びます。MCP toolからlive schedulingを直接開始するtoolはv1では用意していません。live schedulingは既存のn8n workflowまたはGrowth Agent APIから、`AUTO_POSTING_ENABLED=true`, `SCHEDULING_DRY_RUN=false`, `AUTOMATION_KILL_SWITCH=false` が揃った時だけ実行します。

MCP方式でも、以下は変わりません。

- LLMがPostizやXを直接操作しません。
- X APIはread-onlyのままです。
...
| `GROWTH_AGENT_BASE_URL` | CLIとMCP serverが使うAPI base URL。 | CLI / MCP |
| `GROWTH_AGENT_MCP_TIMEOUT_SECONDS` | MCP serverからGrowth Agent APIを呼ぶ時のtimeout。default `10`。 | MCP |
