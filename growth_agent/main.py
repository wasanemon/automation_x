...
from growth_agent.services.metrics import collect_metrics, metrics_summary
...
@app.post("/ideas/ingest", response_model=IdeaResponse, status_code=status.HTTP_201_CREATED)
...
@app.post("/drafts/{draft_id}/evaluate", response_model=EvaluationResponse)
...
@app.post("/drafts/{draft_id}/approve", response_model=DraftResponse)
def approve_draft(
...
@app.post("/drafts/{draft_id}/reject", response_model=DraftResponse)
...
def schedule_draft(
    ...
    draft = _get_draft_or_404(db, draft_id)
    evaluator = DraftEvaluator(get_settings())
    ...
    post = Post(
        ...
        content=draft.content,
        status="scheduled",
        postiz_post_id=scheduled.postiz_post_id,
        postiz_integration_id=scheduled.integration_id,
        scheduled_for=scheduled_for,
        has_url=draft.has_url,
    )
    db.add(draft)
...
@app.post("/posts/reconcile-x-ids", response_model=ReconcileResponse)
def reconcile_x_ids(
...
@app.post("/metrics/collect", response_model=MetricsCollectResponse)
...
@app.post("/feedback/run", response_model=FeedbackRunResponse)
...
@app.get("/feedback/playbook", response_model=list[PlaybookRuleResponse])
